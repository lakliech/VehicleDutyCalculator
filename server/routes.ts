...
Always respond in JSON format. If no specific recommendations, set "recommendations" to an empty array.`
  ...
  // Calculate comprehensive loan details based on listing and product
  app.post('/api/calculate-loan', async (req: Request, res: Response) => {
    try {
      const { listingId, productId, requestedAmount, downPayment, tenure } = req.body;
      
      if (!listingId || !productId) {
        return res.status(400).json({ error: 'Listing ID and Product ID are required' });
      }
      ...
      const product = await db.select({
        ...
        .innerJoin(bankPartners, eq(loanProducts.bankId, bankPartners.id))
        .where(eq(loanProducts.id, parseInt(productId)))
        .limit(1);
      ...
      const vehicle = listing[0];
      const loanProduct = product[0];
      const vehiclePrice = parseFloat(vehicle.price.toString());

      // Calculate loan parameters
      const finalLoanAmount = requestedAmount || (vehiclePrice * parseFloat(loanProduct.maxFinancingPercentage.toString()));
      const finalDownPayment = downPayment || (vehiclePrice * parseFloat(loanProduct.minDownPaymentPercentage.toString()));
      const finalTenure = tenure || loanProduct.maxTenureMonths;
      
      // Use average interest rate if not specified
      const interestRate = (parseFloat(loanProduct.minInterestRate.toString()) + parseFloat(loanProduct.maxInterestRate.toString())) / 2;
      
      // Calculate monthly payment using EMI formula
      const monthlyInterestRate = interestRate / 100 / 12;
      const monthlyPayment = finalLoanAmount * (monthlyInterestRate * Math.pow(1 + monthlyInterestRate, finalTenure)) / (Math.pow(1 + monthlyInterestRate, finalTenure) - 1);
      
      // Calculate total amounts
      const totalPayable = monthlyPayment * finalTenure;
      const totalInterest = totalPayable - finalLoanAmount;
      
      // Calculate fees
      const processingFee = loanProduct.processingFeeFixed ? 
        parseFloat(loanProduct.processingFeeFixed.toString()) : 
        finalLoanAmount * parseFloat(loanProduct.processingFeeRate.toString());
      
      // Estimate insurance cost (2-3% of vehicle value annually)
      const insuranceCost = loanProduct.insuranceRequired ? vehiclePrice * 0.025 : 0;
      
      // Get transfer fees from database
      const transferFees = await db.select().from(transferRates)
        .where(
          and(
            or(
              eq(transferRates.minEngineCapacity, null),
              sql`${vehicle.engineCapacity} >= ${transferRates.minEngineCapacity}`
            ),
            or(
              eq(transferRates.maxEngineCapacity, null),
              sql`${vehicle.engineCapacity} <= ${transferRates.maxEngineCapacity}`
            )
          )
        )
        .limit(1);
      
      const transferFee = transferFees.length > 0 ? parseFloat(transferFees[0].transferFee.toString()) : 5000;
      
      // Registration fees (estimate based on engine capacity)
      const registrationFee = vehicle.engineCapacity <= 1500 ? 3000 : 
                            vehicle.engineCapacity <= 3000 ? 4500 : 6000;
      
      // Total initial costs
      const totalInitialCost = finalDownPayment + processingFee + insuranceCost + transferFee + registrationFee;
      
      // Calculate affordability metrics
      const requiredMonthlyIncome = monthlyPayment * 3; // 33% debt-to-income ratio
      const debtToIncomeRatio = parseFloat(loanProduct.minMonthlyIncome.toString()) > 0 ? 
        (monthlyPayment / parseFloat(loanProduct.minMonthlyIncome.toString())) * 100 : 0;

      // Store calculation in database
      const calculationResult = await db.insert(loanCalculations).values({
        userId: req.user?.id || null,
        sessionId: req.sessionID,
        vehiclePrice: vehiclePrice.toString(),
        downPayment: finalDownPayment.toString(),
        ...
        registrationFees: registrationFee.toString(),
        transferFees: transferFee.toString(),
        totalInitialCost: totalInitialCost.toString(),
      }).returning();

      // Response with comprehensive loan calculation
      res.json({
        ...
        loanProduct: {
          ...
          bankName: loanProduct.bankName,
          interestRate: interestRate,
          minTenure: loanProduct.minTenureMonths,
          maxTenure: loanProduct.maxTenureMonths,
        },
        loanDetails: {
        ...
        affordability: {
          ...
          isAffordable: debtToIncomeRatio <= 33,
        },
        summary: {
          vehiclePrice: Math.round(vehiclePrice),
          totalFinancing: Math.round(finalLoanAmount),
          totalCost: Math.round(totalPayable + totalInitialCost),
          monthlyCost: Math.round(monthlyPayment),
          totalSavings: Math.round(vehiclePrice - finalDownPayment),
        }
      });
    } catch (error) {
      console.error('Error calculating loan:', error);
  ...
  // Get loan calculation history for user
  app.get('/api/loan-calculations', authenticateUser, async (req: Request, res: Response) => {
    try {
      const calculations = await db.select().from(loanCalculations)
        .where(eq(loanCalculations.userId, req.user.id))
        .orderBy(desc(loanCalculations.createdAt))
        .limit(50);

      res.json(calculations);
    } catch (error) {
      console.error('Error fetching loan calculations:', error);
      res.status(500).json({ error: 'Failed to fetch loan calculations' });
    }
  });
